...
@Component
@StepScope
public class InsertaArchivoItemReader extends FlatFileItemReader<RegistroDTO> {
	
	private final static Logger logger = LoggerFactory.getLogger(InsertaArchivoItemReader.class);
	...
	public void init() {
		...
		setLineMapper(lineMapperInsertaArchivo());
		try {
	...
	@Bean
	@StepScope
	public LineMapper<RegistroDTO> lineMapperInsertaArchivo() {
		DefaultLineMapper<RegistroDTO> lineMapper = new DefaultLineMapper<RegistroDTO>();
		lineMapper.setLineTokenizer(productLineTokenizerArchivo());
		lineMapper.setFieldSetMapper(productFieldSetMapperArchivo());
		return lineMapper;
	}

	@Bean
	@StepScope
	public LineTokenizer productLineTokenizerArchivo() {
		FixedLengthTokenizer tokenizer = new FixedLengthTokenizer();
		tokenizer.setColumns(ItemReaderDelimiterEnum.RANGOS_SUI55.getValores());
		tokenizer.setNames(ItemReaderCamposEnum.CAMPOS_SUI55.getValores());
		return tokenizer;
	}

	@Bean
	@StepScope
	public FieldSetMapper<RegistroDTO> productFieldSetMapperArchivo() {
